...
export function usePreferences() {
  ...
  useEffect(() => {
    ...
    const fetchPreferences = async () => {
      const { data, error } = await supabase
        .from('profiles')
        .select('default_face, is_24_hour, show_seconds, sound_enabled, ambient_mode, saved_timezones')
        .eq('user_id', user.id)
        .single();
      ...
      if (data) {
        const dbPrefs: UserPreferences = {
          defaultFace: (data.default_face as WatchFaceType) || DEFAULT_PREFERENCES.defaultFace,
          ...
          ambientMode: data.ambient_mode ?? DEFAULT_PREFERENCES.ambientMode,
          savedTimezones: data.saved_timezones || DEFAULT_PREFERENCES.savedTimezones,
        };
  ...
  const syncToDatabase = useCallback(async (newPrefs: UserPreferences) => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        ambient_mode: newPrefs.ambientMode,
  ...
  const setDefaultFace = useCallback((face: WatchFaceType) => {
  ...
  const toggleAmbientMode = useCallback(() => {
    setPreferences((prev) => {
      const newPrefs = { ...prev, ambientMode: !prev.ambientMode };
      if (user) syncToDatabase(newPrefs);
      return newPrefs;
    });
  }, [user, syncToDatabase]);

  const addTimezone = useCallback((tzId: string) => {
    setPreferences((prev) => {
